...
class BalanceScaleTestCase(unittest.TestCase):
    ...
    def test_compare_return_negative_one_when_left_weighs_more_than_right(self):
        left = [1 ,1 2]
        right = [1, 1 1]
        
        self.assertEqual(self.scale.compare(left, right), -1)

    def test_compare_return_zero_when_left_equals_right(self):
        left = [1 ,1  1]
        right = [1, 1 1]
        
        self.assertEqual(self.scale.compare(left, right), 0)

    def test_compare_return_one_when_left_less_ than_right(self):
        left = [1 ,1  1]
        right = [1, 1 1]
